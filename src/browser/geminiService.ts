...
function getAiClient() {
  ...
  const looksPlaceholder = normalized.includes('your_gemini_api_key_here');
  const looksLikeGoogleKey = /^AIza[A-Za-z0-9_-]{20,}$/.test(normalized);
  if (looksPlaceholder || !looksLikeGoogleKey) {
    throw new Error('Invalid VITE_GEMINI_API_KEY value. Please set a valid Google AI Studio API key.');
  }
...
function toFriendlyErrorHtml(error: unknown) {
  const raw = error instanceof Error ? error.message : String(error);
  const lower = raw.toLowerCase();

  if (lower.includes('api key') || lower.includes('invalid v1 api key') || lower.includes('missing vite_gemini_api_key')) {
    return `
      <div class="p-6 text-red-700 bg-red-50 border border-red-200 rounded-xl max-w-3xl mx-auto mt-6">
        <h1 class="text-2xl font-semibold mb-3">Gemini API Hatasi</h1>
        <p class="mb-3">Gecersiz veya eksik API anahtari nedeniyle sayfa uretilemedi.</p>
        <ol class="list-decimal pl-5 space-y-1 text-sm">
          <li>.env dosyasinda VITE_GEMINI_API_KEY degerini kontrol edin.</li>
          <li>Deger Google AI Studio'dan alinmis gecerli bir anahtar olmali.</li>
          <li>Degisiklikten sonra gelistirme sunucusunu yeniden baslatin.</li>
        </ol>
      </div>
    `;
  }
  ...
  return `
    <div class="p-6 text-red-700 bg-red-50 border border-red-200 rounded-xl max-w-3xl mx-auto mt-6">
      <h1 class="text-2xl font-semibold mb-2">Generation Error</h1>
      <p class="text-sm whitespace-pre-wrap">${escapeHtml(raw)}</p>
    </div>
  `;
}
...
`;
  ...
  try {
    ...
    yield toFriendlyErrorHtml(error);
  }
}